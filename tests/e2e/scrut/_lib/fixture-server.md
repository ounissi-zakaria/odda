...
>   for i in $(seq 1 500); do curl -s -o /dev/null "http://127.0.0.1:$port/" && exit 0; sleep 0.03; done
...
>   for i in $(seq 1 500); do pgrep -f "http.server $port.*$PWD/site" >/dev/null || exit 0; sleep 0.03; done
...
$ wait_for_flow() {
>   marker="$1"
>   for i in $(seq 1 500); do grep -q "$marker" "$PWD/data/flows/flows.jsonl" 2>/dev/null && exit 0; sleep 0.03; done
