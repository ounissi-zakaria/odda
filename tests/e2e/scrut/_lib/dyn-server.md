...
>   for i in $(seq 1 100); do curl -s -k -o /dev/null "https://127.0.0.1:$port/" && exit 0; sleep 0.05; done
...
>   for i in $(seq 1 100); do pgrep -f "hypercorn.*$PWD/dyn.pem" >/dev/null || exit 0; sleep 0.05; done
