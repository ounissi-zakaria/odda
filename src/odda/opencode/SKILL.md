...
- `odda navigate --url <url>` — Navigate an existing tab to a URL. To open a tab, use `odda tabs open`. Text output is the status line (e.g. `Navigated to: <url>`); navigation failures (network error, invalid URL, or `load`-event timeout) print `Error: ...` on stderr with non-zero exit (`--json` gives `{"error": ...}`); the timeout error message points at `odda wait-for` for SPAs whose `load` event never fires.
