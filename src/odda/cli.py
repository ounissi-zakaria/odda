...
Three commands survive the MCP migration (map decision: "CLI survivors"):

- ``mcp`` — the stdio MCP server; the *only* odda process. Spawning it is
  the harness's job (see README for per-harness config).
- ``version`` — version probe for scripts and bug reports.
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    version_p = sub.add_parser("version", help="Print odda version")
...
def _cmd_version(_args: argparse.Namespace) -> int:
    """Print the odda version."""
    print(f"odda {__version__}")
    return 0
