...
`--line-terminator` sets the byte sequence the request parser splits header lines on (default `\r\n`). The block terminator (header/body separator) is always two line terminators. Set it via `request new --line-terminator <bytes>` (accepts backslash escapes: `\n`, `\x00`, ...); it is stored in `meta.json` as `line_terminator` (a list of byte ints) and round-trips on `send`.

**H2-only.** The line terminator is honored only for HTTP/2 request files. HTTP/1.1 request files are wire-faithful — the bytes go on the socket verbatim, so re-framing them is meaningless. Setting `--line-terminator` on an H1 request has no effect (the parser always uses `\r\n` for H1).
...
```bash
$ odda request new --name h2smuggle --host <lab> --port 443 --line-terminator '\x00'
$ printf 'GET /foo\r\nX-Evil:yes HTTP/2\x00Host: <lab>\x00\x00' \
  > .odda/requests/h2smuggle/request
$ odda request send --name h2smuggle --insecure
```
