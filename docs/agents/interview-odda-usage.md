...
You are being interviewed about your experience using the `odda` CLI for browser automation, HTTP traffic capture, dynamic analysis, and raw request crafting. The goal is to find rough edges so the tool can be improved. Be specific and honest — "it worked fine" is not useful; concrete commands, exact errors, and the JSON you got back are.
...
First, capture the environment so reports are reproducible — paste the output of `odda version` and `odda status`, and note the OS and Chrome version if you know them.

For each problem, give: the exact command(s) you ran (in a fenced code block), the error or JSON you got back, what you expected, and what you did to work around it. Include non-zero-exit errors, hangs, wrong results, and anything that felt like a bug even if you're unsure.
...
## 4. Docs & SKILL.md
- Was there anything you couldn't find in the skill docs and had to discover by trial and error?
- Did you use the linked reference docs (`FLOWS.md`, `USERSCRIPTS.md`, `DYNAMIC-ANALYSIS.md`, `REQUEST.md`, `recipes/`)? If yes, were they accurate? If no — did you know they existed?
...
- **Targeting model & IDs:** Did the `--browser-id` / `--tab-id` model make sense? Any stale-ID or wrong-tab confusion? Multi-agent/shared-server scenarios?
...
- **`eval` / `wait-for`:** Serialization surprises (double-encode, `Response` → `{}`, Promises), expression-vs-return confusion, timeout behavior?
...
- **Raw request crafting:** `request clone`/`new`/`send`, CRLF gotchas, `meta.json` host vs `Host` header, HTTP/2 framing, `--fix-content-length`?
- **Server / plugin:** Did the server start cleanly via the OpenCode plugin? Were `ODDA_SOCKET` / `ODDA_DATA_DIR` present? Any `odda status` / `odda logs` issues?
...
- Any operation that was noticeably slow (single command > 5s, or repeated commands adding up)? Note the command and rough wall-clock time.
