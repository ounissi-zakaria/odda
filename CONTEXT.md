...
**Screenshot**:
A JPEG of a tab's viewport returned by the `screenshot` tool — pixels for reading visual layout, the counterpart of a **Snapshot** (which names actionable elements). An annotated Screenshot draws every **Ref**'s bounding box, label, and origin coordinates onto the pixels — the visual counterpart of a Snapshot's boxes, pairing pixels with refs and coordinates for **Coordinate targeting**; annotation is drawn onto the captured pixels, never by injecting anything into the page. The written path is always the result's text handle; the pixels are additionally returned inline by default, with `return_image=False` collapsing the result to the path alone.
